...
function selectTool(name) {
  ...
  if (name !== currentTool) ToolRegistry.deactivate(currentTool);
  ...
  if (_txp) { _txp.classList.remove('is-open'); _txp.hidden = true; }
  ...
  workspace.style.cursor = name === 'text' ? 'text' : 'crosshair';
...
function setShapeType(type) {
  ...
  // Forward to ShapeTool so the new smart-object module sees the active kind
  // (corner-radius option-group visibility depends on this).
  if (window.ShapeTool && window.ShapeTool.setActiveShapeType) {
    window.ShapeTool.setActiveShapeType(type);
  }
}
